...
def font(name_substr: str, size: int) -> ImageFont.FreeTypeFont:
    ...
    if p and p.exists():
        try:
            ...
            pass
    for fb in ["Songti.ttc", "STHeiti Medium", "STHeiti", "Times"]:
        p = find_font(fb)
        if p and p.exists():
            try:
                return ImageFont.truetype(str(p), size=size)
...
def draw_stamp_circle(draw, cx: int, cy: int, radius: int, char: str,
    ...
    stamp_font = font("Songti.ttc", int(radius * 1.5))
...
def draw_lede_with_dropcap(draw, lede: str, x: int, y: int, max_w: int,
    ...
    rest = lede[1:]
    cap_size = 60
    cap_font = font("Songti.ttc", cap_size)
    line_h = 44
...
def render_card(theme: dict, slug: str) -> None:
    ...
    title_font = font("Songti.ttc", title_size)
