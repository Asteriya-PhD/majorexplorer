...
> 时间戳: 2026-06-12 (最后更新) · 上一版 2026-06-10

## -1. 2026-06-12 — 部署规划 v2 大改 (Cloudflare Pages 海外直链)

**Commits**: 待 push (Cloudflare 注册中, 等前端隔壁修复完成一起 push)

### 战报

| 指标 | v1 (ADR-011~015) | v2 (ADR-016~019) | 变化 |
|---|---|---|---|
| 域名 TLD | .cn 双持 | **.com 单持** (`majorexplorer.com`) | 大改 |
| 静态托管 | EdgeOne Pages 两阶段 | **Cloudflare Pages 单阶段** | 大改 |
| 国内访问 | EdgeOne 海外 30-80ms | **Cloudflare 21 接入商 + 优选 IP 30-100ms** | 持平 |
| ICP 备案 | 7-15 工作日 (阿里云轻量) | **不做** (短期) | 大改 |
| 商业化 | 微信/SEO/广告全开 | **纯公益, 全关闭** | 大改 |
| 教师身份 | (待决策) | **不暴露, 普通个人备案备用** | 锁定 |
| 上线时间 | 备案后 6/30 前后 | **6/12 当天** (抢出分前 11 天) | **-18 天** |
| 月成本 | 0-100 元 | **0 元** (免费层 + LLM 后端 0.5 元) | -50% |

### 5 项重大变动

1. **域名**: 2 字中文 .cn 全部被注 → 改 `majorexplorer.com` (跟 `index.html` 品牌一致)
2. **托管**: EdgeOne Pages → **Cloudflare Pages** (国内 21 接入商节点, 优选 IP 30-100ms)
3. **备案**: 7-15 天 → **不做** (公益不变现, 备案价值抵不上时间成本)
4. **微信**: 假设必做 → **不做** (公益项目, 教师身份不暴露策略)
5. **SEO**: 假设要做长尾 → **不做** (一年几天高峰, ROI 低)

### 新增 ADR (4 个)

- **ADR-016**: 域名改 `majorexplorer.com` (.com 单持)
- **ADR-017**: 静态托管改 Cloudflare Pages (弃 EdgeOne Pages)
- **ADR-018**: 短期不做 ICP 备案
- **ADR-019**: 教师身份不暴露, 普通个人备案 (留作未来)

### 旧 ADR 状态变更 (3 个)

- **ADR-011** (EdgeOne Pages 两阶段) → 🕐 过期, 被 ADR-017 替代
- **ADR-013** (阿里云轻量备案) → 🕐 过期, 被 ADR-018 替代
- **ADR-015** (部署实施延后) → 🔄 部分解锁 (前端 v1 基本定型, 静态站可启动)

### 配套新建 (用户回来前 AI 准备)

- ✅ `docs/DEPLOYMENT.md` v2 全文重写 (v1 替换)
- ✅ `docs/DECISIONS.md` 新增 ADR-016/017/018/019
- ✅ `docs/PROGRESS.md` 本节更新 (本文)
- 🔄 `deploy/cloudflare-pages.md` Cloudflare Pages 部署操作指南
- 🔄 `deploy/optimal-cf-ip.sh` 优选 IP + DNS API 同步脚本

### 联调时间线

- **用户**: Cloudflare Registrar 注册 `majorexplorer.com` (进行中)
- **AI**: 文档 + 部署指南 + 优选 IP 脚本 (本次会话)
- **联调**: 用户 git push → Cloudflare Pages 自动部署 → 验证 → 优选 IP → 完成
- **目标**: 6/12 当天 `https://majorexplorer.com` 可访问, 抢 6/13-6/25 高考结束-出分空档

### 风险与缓解 (新场景)

| 风险 | 概率 | 缓解 |
|------|------|------|
| Cloudflare 节点晚高峰跳美西 | 中 | 优选 IP 5 个, DNS TTL 300s 切换 |
| 域名被抢注 | 极低 | 6/12 已注册, 续费不涨价 |
| 备案触发后流程 | 低 (短期不触发) | ADR-019 普通个人备案策略 |
| 教师身份暴露 | 低 | 备案 + 站内容都不写教师 |
| 突发流量 | 低 | Cloudflare Pages 免费层 5 万次/月 |
