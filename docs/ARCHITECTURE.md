...
```
                    ...
                    └────────┬─────────┘
                             ...
                             ▼
                    ┌──────────────────┐
                    │ core/recommender │  ←──── strategy (advisory only)
                    └────┬───┬───┬─────┘
                         │   │   │
        ┌────────────────┘   │   └─────────────────┐
        ▼                    ▼                     ▼
┌──────────────┐    ┌──────────────┐      ┌──────────────────┐
│ core/filter  │    │ core/        │      │ core/data_loader │
│              │    │ probability  │      │  (lru_cache)     │
└──────┬───────┘    └──────┬───────┘      └────────┬─────────┘
       │                   │                      │
       │                   ▼                      ▼
       │            ┌──────────────┐      ┌──────────────────┐
...
|---|---|---|---|
| POST | `/api/score-to-rank` | `{province, score}` | `{province, score, rank}` |
| POST | `/api/rank-to-score` | `{province, rank}` | `{province, rank, score}` |
| POST | `/api/equivalent` | `{province, rank, subject, year}` | `{equivalent_scores: {year: score}}` |
| GET | `/api/meta?province=` | — | `{provinces, subjects, years, xuanke_options, current_province}` |
| POST | `/api/recommend` | `RecommendRequest` (17 fields) | `RecommendResponse.model_dump()` (404 with fetch-script hint on FileNotFoundError) |
| POST | `/api/recommend/pdf` | `RecommendRequest` | `application/pdf` (Content-Disposition: `volunteer_report_{prov}_{subj}_{year}.pdf`) |
| GET | `/` | — | `frontend/index.html` (if exists) |
...
```python
def estimate_admission_probability(student_rank, school, group, prov, subj):
    median_rank, historical_ranks = historical_min_rank(...)
    if no_history: return 0.5, warning
    σ = max(0.25 * median_rank, 1)         # 浮动 25%
    z = (student_rank - median_rank) / σ
    P = 0.5 * (1 + erf((-z + 0.7) / √2))   # Gaussian CDF
